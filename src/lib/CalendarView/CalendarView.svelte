...
>
	<header class="calendar-view-header">
		<div class="calendar-view-header-text" role="heading" aria-level="2" aria-live="polite">
			<button
		...
		<div class="calendar-view-pagination-controls">
			<button type="button" disabled={view && min >= page} aria-label="Previous page" onclick={() => updatePage(-1)}>
			...
			<button type="button" disabled={max < nextPage} aria-label="Next page" onclick={() => updatePage(1)}>
				<svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
					<path
